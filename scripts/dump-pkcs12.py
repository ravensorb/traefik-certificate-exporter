...
argParser = argparse.ArgumentParser(
    prog="dump-pkcs12", description="Convert PEM cert and key to PKCS12"
)
argParser.add_argument(
    "-c", "--cert-file", dest="cert_file", default="fullchain.pem", help="PEM cert file"
)
argParser.add_argument(
    "-k", "--key-file", dest="key_file", default="privkey.pem", help="PEM key file"
)
argParser.add_argument(
    "-p",
    "--passphrase",
    dest="passphrase",
    default=None,
    help="Passphrase for the PKCS12 file",
)
argParser.add_argument(
    "-o",
    "--output-file",
    dest="output_file",
    default="cert-temp.pfx",
    help="PKCS12 output file",
)
...
(pkcs12_key, pkcs12_cert, pkcs12_chain) = pkcs12.load_key_and_certificates(
    pkcs12_bytes,
    args.passphrase.encode("utf-8") if args.passphrase is not None else None,
)
...
print(
    "  Subject: {}".format(
        str(
            pkcs12_cert.subject.get_attributes_for_oid(x509.oid.NameOID.COMMON_NAME)[
                0
            ].value
        )
    )
...
print(
    "  Issuer: {}".format(
        str(
            pkcs12_cert.issuer.get_attributes_for_oid(x509.oid.NameOID.COMMON_NAME)[
                0
            ].value
        )
    )
