...
## Epic 10: Secret Redaction, and Post-Export Notifications

Credentials held as configuration *values* stop reaching the logs, and operators learn that
certificates were exported — by email, chat, or a webhook — without this project implementing a
delivery mechanism for any channel.

Decided in ADR-0014, which supersedes ADR-0013 (withdrawn before implementation; its postmortem
records why). Two architecture gates shaped this: the first cut 145–161 hours to 10 by removing
scope nobody asked for, the second split the redaction work out and corrected three factual
claims. Both gate reviews are under `epic-010/arch/`.

**Dependencies:** Epic 5 (the post-export hook this sits beside).

### Story 10.1: Redact Secrets Held as Configuration Values
...
### Story 10.2: Notify Apprise Destinations After an Export Pass
...
**Then** each destination is notified with the exported domain list, and a delivery failure is
logged **as a delivery failure** — never propagating, and never mislabelled as an export error by
the drain's per-path handler

**Given** `Apprise.notify()` takes no timeout and bounds delivery per destination instead
(`cto`/`rto`, defaulting to 4.0s connect + 4.0s read; the SMTP plugin overrides connect to 15s)
**When** the worst-case contribution to the drain's blind window is documented
**Then** it is stated as the **sum over configured destinations**, not a single number, because
with N destinations there is no single timeout to quote — and a test asserts that figure is
derived from the configured set rather than hard-coded
